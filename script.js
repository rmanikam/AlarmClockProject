...
function addAlarmAnddeletebuttom(
  hourSelection,
  minSelection,
  secSelection,
  ampmval,
  ...
  newParaTah.innerText =
    hourSelection + ":" + minSelection + ":" + secSelection + "" + ampmval;
...
AlarmSelection.addEventListener("click", function () {
  let hourSelection = document.getElementById("hourSelction").selectedIndex;
  ...
  minSelection;
  secSelection;
  hourSelection = hourSelection < 10 ? "0" + hourSelection : hourSelection;
  ...
  let alarmTimes = hourSelection + minSelection + secSelection + ampmval;
  ...
  arr.filter(function (item) {
    var i = resArr.findIndex(
      (x) =>
        x.hourSelection == item.hourSelection &&
        x.minSelection == item.minSelection &&
        x.secSelection == item.secSelection
    );
    if (i <= -1) {
      resArr.push(item);
    }
